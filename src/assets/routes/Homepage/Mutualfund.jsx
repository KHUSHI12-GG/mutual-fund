...
const MutualFundDashboard = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(2); // Center index
  ...
  const goToNext = () => {
    setCurrentIndex((prevIndex) => 
      prevIndex === funds.length - 1 ? 0 : prevIndex + 1
    );
  };
      ...
      >
        <div className="relative h-[500px] flex items-center justify-center">
          ...
          <button 
            ...
            className="absolute left-0 z-20 bg-white/80 hover:bg-white p-3 rounded-full shadow-lg transition-all transform hover:scale-110"
          >
          ...
          <button 
            onClick={goToNext}
            className="absolute right-0 z-20 bg-white/80 hover:bg-white p-3 rounded-full shadow-lg transition-all transform hover:scale-110"
          >
            <svg className="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {funds.map((fund, index) => {
            ...
            if (position === 0) {
              ...
              scale = 1;
              zIndex = 30;
            } else if (position === -1 || (currentIndex === 0 && index === funds.length - 1)) {
              // Left item
              ...
              blur = 'blur(1px)';
              rotation = '-5deg';
            } else if (position === 1 || (currentIndex === funds.length - 1 && index === 0)) {
              // Right item
              transform = 'translateX(80%)';
              scale = 0.8;
              zIndex = 20;
              ...
              // Hidden items
              transform = position < 0 ? 'translateX(-150%)' : 'translateX(150%)';
              opacity = 0;
              scale = 0.7;
              zIndex = 10;
            }
            ...
            return (
              <div
                key={fund.id}
                className={`absolute w-4/5 max-w-lg transition-all duration-700 ease-in-out transform-gpu ${
                  position === 0 ? 'cursor-pointer' : ''
                }`}
                style={{
                  transform: `${transform} scale(${scale}) rotateY(${rotation})`,
                  opacity: opacity,
                  filter: blur,
                  zIndex: zIndex,
                }}
                onClick={() => position === 0 && openDetailsForm()}
              >
                <div className={`bg-gradient-to-br ${
                  ...
                  <div className="flex items-center mb-4">
                    <div className="bg-white/20 p-3 rounded-full mr-4">
                      <span className="text-white text-2xl font-bold">{fund.name.charAt(0)}</span>
                    </div>
                    <div>
                      <h2 className="text-xl font-bold text-white">{fund.name}</h2>
                      <div className="flex items-center mt-1">
                        <span className="text-white/80">{fund.category}</span>
                        <span className="mx-2 text-white/50">•</span>
                        <span className="text-white/90 font-medium">{fund.risk} Risk</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3">
                      <p className="text-white/80 text-sm">Fund Size</p>
                      <p className="font-semibold text-white text-lg">{formatCurrency(fund.size)}</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3">
                      <p className="text-white/80 text-sm">Min. Investment</p>
                      <p className="font-semibold text-white text-lg">{formatCurrency(fund.minInvestment)}</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3">
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3">
                      <p className="text-white/80 text-sm">CAGR</p>
                      <p className="font-semibold text-green-300 text-xl">{fund.cagr}%</p>
                    </div>
                  </div>
                </div>
              </div>
